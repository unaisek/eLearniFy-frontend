...
export class CourseService {
  ...
  private _courseDetailsSubject: BehaviorSubject<ICourse| null> = new BehaviorSubject<ICourse | null>(null);
  public courseDetails$: Observable<ICourse | null> = this._courseDetailsSubject.asObservable();
  ...
  getCourseDetails(courseId: string): void {    
    this._http.get<ICourse>(`${env.apiUrl}/tutor/view-course/${courseId}`).subscribe((courseData)=>{     
  ...
  addNewChapter(formData:FormData,courseId:string):Observable <any>{
    return this._http.post(`${env.apiUrl}/tutor/add-chapter/${courseId}`,formData)
  }
